...
import { BarChart3, Map, Target, Link, ArrowRight, CheckCircle, Activity } from 'lucide-react';
import { SolutionNarrativeSectionData } from '@/types/educational-capability';

interface PathwaySolutionInteractiveProps {
...
const solutionSteps: SolutionStep[] = [
  {
    id: 'pathway-aggregation',
    number: 1,
    title: 'Pathway Aggregation',
    description: 'Aggregates sequence disruption scores into pathway-level signals using weighted gene-to-pathway mappings',
    icon: BarChart3,
    color: 'blue',
    details: [
      { label: 'Method', value: 'Weighted averaging of sequence scores' },
      { label: 'Formula', value: 'pathway_score = sum(sequence_disruption × weight) / count' },
      { label: 'Integration', value: 'Called from efficacy orchestrator during S/P/E pipeline' },
      { label: 'Weight', value: '40% of S/P/E framework' },
    ],
    metrics: [
      { label: 'S/P/E Weight', value: '40%' },
      { label: 'Pathways', value: '5' },
    ],
  },
  {
    id: 'gene-pathway-mapping',
    number: 2,
    title: 'Gene-to-Pathway Mapping',
    description: 'Maps cancer genes to biological pathways with transparent weights for mechanism-based prediction',
    icon: Map,
    color: 'green',
    details: [
      { label: 'Pathways', value: 'DDR, MAPK, TP53, PI3K, VEGF' },
      { label: 'Key Genes', value: 'BRCA1/2, BRAF/KRAS, TP53, PIK3CA, VEGFA' },
      { label: 'Weight System', value: 'Binary weights (1.0) for primary pathway' },
      { label: 'Extensibility', value: 'Easy addition of new genes and pathways' },
    ],
    metrics: [
      { label: 'Pathways', value: '5' },
  ...
  {
    id: 'drug-pathway-alignment',
    number: 3,
    title: 'Drug-to-Pathway Alignment',
    description: 'Pre-configured drug panels with pathway weight mappings for mechanism-based therapy selection',
    icon: Target,
    color: 'purple',
    details: [
      { label: 'Disease Panels', value: 'MM, Ovarian, Melanoma' },
      { label: 'Pathway Weights', value: 'Drug-specific pathway relevance weights' },
      { label: 'MoA Integration', value: 'Mechanism-of-action annotations for each drug' },
      { label: 'Consistency', value: 'Standardized weights across disease panels' },
    ],
    metrics: [
      { label: 'Panels', value: '3' },
      { label: 'Drugs', value: '15+' },
    ],
  },
  {
    ...
    color: 'orange',
    details: [
      { label: 'Integration', value: 'Pathway scores feed into SL analysis' },
      { label: 'Double-Hit Detection', value: 'Identifies vulnerabilities from pathway disruptions' },
      { label: 'Status', value: '✅ Complete integration' },
      { label: 'Framework', value: 'S/P/E + Synthetic Lethality' },
    ],
    metrics: [
...
export default function PathwaySolutionInteractive({ data, className = '' }: PathwaySolutionInteractiveProps) {
  const [activeStep, setActiveStep] = useState<string>('pathway-aggregation');
  const selectedStep = solutionSteps.find(s => s.id === activeStep) || solutionSteps[0];

  return (
    <section className={`py-16 px-4 md:px-8 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 ${className}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          ...
          className="mt-8 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl p-6 border-2 border-indigo-200"
        >
          ...
          <div className="grid md:grid-cols-4 gap-4">
            {[
              { icon: BarChart3, label: 'Pathway Aggregation', value: '100% integration' },
              { icon: Map, label: 'Gene Mapping', value: '5 pathways' },
              { icon: Target, label: 'Disease Panels', value: '3 panels' },
              { icon: Link, label: 'SL Integration', value: '✅ Complete' },
            ].map((moat, idx) => {
              const MoatIcon = moat.icon;
              return (
                <div key={idx} className="bg-white rounded-xl p-4 border-2 border-indigo-200">
                  <div className="flex items-center gap-3 mb-2">
                    <MoatIcon className="w-5 h-5 text-indigo-600" />
        ...
        >
          <span className="text-indigo-600">This is the first system that transforms</span>
          <br />
          <span className="text-slate-900">variant-level scores into pathway-level drug efficacy predictions.</span>
        </motion.p>
      </div>
    </section>
  );
