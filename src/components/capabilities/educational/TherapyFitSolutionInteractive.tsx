...
import { Zap, Shield, Activity, CheckCircle, ArrowRight, Target, TrendingUp, BarChart3, Link } from 'lucide-react';
import { SolutionNarrativeSectionData } from '@/types/educational-capability';

interface TherapyFitSolutionInteractiveProps {
...
const solutionSteps: SolutionStep[] = [
  {
    ...
    description: 'Intelligent S/P/E fusion (Sequence, Pathway, Evidence) with confidence scoring and clear biological rationale',
    icon: Zap,
    color: 'blue',
    details: [
      { label: 'Validation', value: '95.7% AUROC ClinVar (53,210 variants)' },
      { label: 'Fusion', value: 'S/P/E provides biological rationale' },
      { label: 'Confidence', value: '70-85% confidence scoring' },
      { label: 'Time Reduction', value: 'Days → Minutes' },
    ],
    metrics: [
      { label: 'AUROC', value: '95.7%' },
      { label: 'Variants', value: '53,210' },
    ],
  },
  {
    id: 'biology-alignment',
    number: 2,
    title: 'Biology-Therapy Alignment',
    description: 'Systematic connection between patient genetics, pathway biology, and drug mechanisms of action',
    icon: Shield,
    color: 'green',
    details: [
      { label: 'Missed Opportunities', value: 'Reduced by 85%' },
      { label: 'Therapy Relevance', value: 'Improved by 75%' },
      { label: 'Selection Time', value: 'Reduced by 60-65%' },
      { label: 'Consistency', value: 'Across similar profiles' },
    ],
    metrics: [
  ...
  },
  {
    ...
    description: 'Comprehensive evidence integration from ClinVar, literature, and clinical data with transparent badges',
    icon: Activity,
    color: 'purple',
    details: [
      { label: 'Incomplete Assessments', value: 'Reduced by 90%' },
      { label: 'Data Capture', value: '95% of relevant data' },
      { label: 'Evidence Interpretation', value: 'Improved by 70%' },
      { label: 'Risk Reduction', value: '60% lower risk' },
    ],
    metrics: [
      { label: 'Capture', value: '95%' },
      { label: 'Improvement', value: '70%' },
    ],
  },
  {
    id: 'documentation',
    ...
    description: 'Complete audit trail with run IDs, citations, and provenance tracking for full reproducibility',
    icon: CheckCircle,
    color: 'orange',
    details: [
      { label: 'Audit Trail', value: '100% with run ID tracking' },
      { label: 'Review & Validation', value: 'Improved by 90%' },
      { label: 'Provenance', value: 'Complete tracking' },
      { label: 'Standards', value: 'Research-grade methodology' },
    ],
    metrics: [
...
export default function TherapyFitSolutionInteractive({ data, className = '' }: TherapyFitSolutionInteractiveProps) {
  const [activeStep, setActiveStep] = useState<string>('drug-ranking');
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-lg text-slate-700 max-w-3xl mx-auto">
            Turn genetics into a plain, ranked therapy view you can act on: top classes, a short 'why,' confidence, and a shareable, source‑backed one‑pager.
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="grid md:grid-cols-4 gap-4">
            {[
              { icon: TrendingUp, label: 'Tier Promotions', value: '30-45% improvement' },
              { icon: Shield, label: 'Confidence Enhancement', value: '+0.08 median lift' },
              { icon: BarChart3, label: 'Ranking Stability', value: '95% consistency' },
              { icon: Link, label: 'Fusion Integration', value: '+0.03-0.07 lift' },
            ].map((moat, idx) => {
              const MoatIcon = moat.icon;
              return (
                <div key={idx} className="bg-white rounded-xl p-4 border-2 border-blue-200">
                  <div className="flex items-center gap-3 mb-2">
                    <MoatIcon className="w-5 h-5 text-blue-600" />
        ...
        >
          <span className="text-blue-600">This is the first system that connects</span>
          <br />
          <span className="text-slate-900">genetics to ranked, actionable therapy recommendations.</span>
        </motion.p>
      </div>
    </section>
  );
