...
export const TransparencyComparisonCards: React.FC<TransparencyComparisonCardsProps> = ({ 
  variant = 'section',
  className = '' 
}) => {
  ...
  return (
    <div className={`${isSlide ? 'w-full h-full' : ''} ${className}`}>
      <div className={`${isSlide ? 'h-full' : ''} grid grid-cols-2 gap-2 sm:gap-4 md:gap-6 lg:gap-8 items-stretch`}>
        {/* Left: Problem */}
        <motion.div
