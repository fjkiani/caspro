          ...
          <section className="border border-white/10 rounded p-5 bg-white/[0.02]">
            {tab === 'formula' && (
              <div className="space-y-3">
                ...
                <p className="font-mono text-sm bg-black/40 border border-white/10 rounded px-3 py-2">{g.formula || 'not set'}</p>
                <div className="text-[10px] uppercase tracking-widest text-white/50">
                  status · {g.formula_status || 'unknown'} · signed · {g.formula_signed || 'unsigned'}
                </div>
              </div>
            )}
            {tab === 'quarantine' && (
            ...
            {tab === 'remediation' && (
              <div className="space-y-3">
                ...
                <p className="text-sm text-white/80 whitespace-pre-line">{g.remediation || 'no remediation recorded'}</p>
              </div>
            )}
          </section>
        </div>
