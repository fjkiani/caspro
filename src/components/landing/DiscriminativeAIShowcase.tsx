  ...
  return (
    <div className={`grid grid-cols-1 lg:grid-cols-2 gap-8 ${className}`}>
      {Object.entries(endpoints).map(([key, endpoint]) => {
        ...
        return (
          <div key={key} className="bg-white border border-slate-200 rounded-2xl p-8 hover:shadow-lg transition-shadow">
            {/* Card Header */}
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className={`text-slate-500 font-mono ${getTextSize('text-sm')}`}>
                    /{endpoint.id}
                  </p>
                </div>
              </div>
            ...
            <p className={`text-slate-600 mb-6 leading-relaxed ${getTextSize('text-base')}`}>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              {topMetrics.map((metric, index) => (
                <div key={index} className="text-center bg-slate-50 rounded-lg p-3 border border-slate-200">
                  <div className={`font-bold ${colors.text} ${getTextSize('text-lg')}`}>
                    {metric.value}
                  </div>
                  <div className={`text-slate-600 ${getTextSize('text-xs')}`}>
                    {metric.label}
                  </div>
                </div>
              ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {endpoint.outputs.map((output: string, index: number) => (
                      <span 
                        key={index}
                        className={`px-3 py-1 bg-slate-100 border ${colors.border} border-opacity-50 rounded-full font-mono ${getTextSize('text-xs')} text-slate-700`}
                      >
                        {output}
                      </span>
                    ))}
                        ...
                        <div className="space-y-2">
                          {(endpoint.useCases[activeUseCase as keyof typeof endpoint.useCases] as readonly string[]).slice(0, 4).map((application, index) => (
                            <div key={index} className="flex items-start gap-2">
                              ...
                              <span className={`text-slate-700 ${getTextSize('text-sm')}`}>{application}</span>
                            </div>
                          ))}
                        </div>
                      </div>
