...
import { ChevronRight, X, Play, CheckCircle, Clock, Zap, Activity, ShieldCheck, Info } from 'lucide-react';
...
const StepDetailModal = ({ step, onClose, getTextSize = (s: string) => s }: { 
  step: AnalysisStep; 
  onClose: () => void;
  getTextSize?: (size: string) => string;
}) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border-2 border-slate-300 dark:border-slate-600 shadow-2xl">
        {/* Header */}
        <div className="sticky top-0 bg-slate-100 dark:bg-slate-800 border-b border-slate-300 dark:border-slate-600 p-6 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className={`text-slate-600 dark:text-slate-300 font-mono ${getTextSize('text-sm')}`}>
                {step.endpoint}
              </p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {typeof step.evidence.conservation === 'number' && (
                  <div className="p-3 bg-slate-50 dark:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-600">
                    <div className={`text-slate-500 dark:text-slate-400 ${getTextSize('text-xs')}`}>Conservation (proxy)</div>
                    <div className={`font-semibold text-slate-900 dark:text-white ${getTextSize('text-base')}`}>{(step.evidence.conservation * 100).toFixed(0)}%</div>
                  </div>
                )}
                {step.evidence.domainHits && step.evidence.domainHits.length > 0 && (
                  <div className="p-3 bg-slate-50 dark:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-600">
                    <div className={`text-slate-500 dark:text-slate-400 ${getTextSize('text-xs')}`}>Domain hits</div>
                    <div className={`font-semibold text-slate-900 dark:text-white ${getTextSize('text-sm')}`}>{step.evidence.domainHits.join(', ')}</div>
                  </div>
                )}
                {step.evidence.motifHits && step.evidence.motifHits.length > 0 && (
                  <div className="p-3 bg-slate-50 dark:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-600">
                    <div className={`text-slate-500 dark:text-slate-400 ${getTextSize('text-xs')}`}>Motif hits</div>
                    <div className={`font-semibold text-slate-900 dark:text-white ${getTextSize('text-sm')}`}>{step.evidence.motifHits.join(', ')}</div>
                  </div>
                )}
                {step.evidence.saeTags && step.evidence.saeTags.length > 0 && (
                  <div className="p-3 bg-slate-50 dark:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-600">
                    <div className={`text-slate-500 dark:text-slate-400 ${getTextSize('text-xs')}`}>SAE tags</div>
                    <div className={`font-semibold text-slate-900 dark:text-white ${getTextSize('text-sm')}`}>{step.evidence.saeTags.join(', ')}</div>
                  </div>
                )}
              </div>
          ...
          <div>
            ...
            <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-4 border border-slate-200 dark:border-slate-600">
              <pre className={`text-slate-700 dark:text-slate-200 font-mono ${getTextSize('text-sm')}`}>
          ...
          <div>
            ...
            <div className="space-y-4">
              {step.processingSteps.map((processStep, index) => (
                <div key={index} className="bg-slate-50 dark:bg-slate-700 rounded-lg p-4 border border-slate-200 dark:border-slate-600">
                  <div className="flex items-center justify-between mb-2">
                    <h5 className={`font-semibold text-slate-900 dark:text-white ${getTextSize('text-base')}`}>
                      {processStep.name}
                    </h5>
                    <span className={`text-${step.color}-600 dark:text-${step.color}-400 font-mono ${getTextSize('text-sm')}`}>
                      {processStep.duration}ms
                    </span>
                  </div>
                  <p className={`text-slate-600 dark:text-slate-300 mb-3 ${getTextSize('text-sm')}`}>
                  ...
                  <div className="space-y-1">
                    {processStep.details.map((detail, detailIndex) => (
                      <div key={detailIndex} className="flex items-start gap-2">
                        ...
                        <span className={`text-slate-600 dark:text-slate-300 ${getTextSize('text-xs')}`}>
                          {detail}
                        </span>
                      </div>
                    ))}
          ...
          {step.provenance && (
            <div className="p-3 bg-slate-50 dark:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-600">
              <div className={`text-slate-500 dark:text-slate-400 ${getTextSize('text-xs')}`}>Provenance</div>
              <div className={`font-semibold text-slate-900 dark:text-white capitalize ${getTextSize('text-sm')}`}>{step.provenance.replace('-', ' ')}</div>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {currentStep >= 0 && currentStep < steps.length && (
        <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl p-6 border-2 border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white">
            ...
            <div>
              ...
              <div className="bg-white dark:bg-slate-700 rounded-lg p-3 border border-slate-200 dark:border-slate-600">
                <div className="text-center py-4">
                  ...
                  <p className={`text-slate-600 dark:text-slate-300 mt-2 ${getTextSize('text-sm')}`}>
                    Ready to analyze
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-700 rounded-lg p-3 border border-slate-200 dark:border-slate-600">
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span className={`text-slate-600 dark:text-slate-300 ${getTextSize('text-sm')}`}>
                      accessibility:
                    </span>
                    <span className={`font-bold text-slate-900 dark:text-white ${getTextSize('text-sm')}`}>0.820</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={`text-slate-600 dark:text-slate-300 ${getTextSize('text-sm')}`}>
                      state:
                    </span>
                    <span className={`font-bold text-slate-900 dark:text-white ${getTextSize('text-sm')}`}>Open_Chromatin</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={`text-slate-600 dark:text-slate-300 ${getTextSize('text-sm')}`}>
                      tissueSpecificity:
                    </span>
                    <span className={`font-bold text-slate-900 dark:text-white ${getTextSize('text-sm')}`}>0.910</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {steps.filter(step => step.status === 'completed').map((step) => (
            <button
              ...
              className="flex items-center gap-4 p-4 bg-white dark:bg-slate-800 rounded-lg border-2 border-slate-200 dark:border-slate-600 hover:border-blue-500 transition-all duration-200 text-left group hover:shadow-lg"
            >
              ...
              <div className="flex-1">
                ...
                <p className={`text-slate-600 dark:text-slate-300 font-mono ${getTextSize('text-xs')}`}>
                  {step.endpoint}
                </p>
              </div>
              <div className="flex items-center gap-2">
