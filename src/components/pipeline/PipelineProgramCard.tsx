      ...
      <div className="mt-5">
        {tab === 'overview' && (
          <div className="space-y-4 text-sm text-white/80">
            ...
            {p.key_findings.slice(0, 3).map((f, i) => (
              <p key={i} className="border-l-2 border-cyan-500/40 pl-3">{f}</p>
            ))}
          </div>
        )}
        ...
        {tab === 'findings' && (
          <ul className="space-y-2 text-sm text-white/80 list-disc pl-5">
            {p.key_findings.map((f, i) => <li key={i}>{f}</li>)}
            {p.key_findings.length === 0 && <li className="list-none text-white/40">No findings recorded.</li>}
          </ul>
        )}
