...
const InSilicoCapabilityCard: React.FC<InSilicoCapabilityCardProps> = ({ capability }) => {
  ...
  return (
    <Link href={`/platform/${capability.coPilotSlug}`}>
      <motion.div
        className={`relative overflow-hidden bg-white rounded-3xl p-8 border-2 ${theme.border} shadow-lg hover:shadow-2xl transition-all duration-500 group cursor-pointer`}
      ...
      {/* Icon */}
      <div className={`inline-flex p-4 rounded-2xl mb-6 ${theme.bg} ${theme.hover} transition-colors duration-300`}>
      ...
      {/* Core Capability */}
      <div className="mb-6">
        <h4 className="text-sm font-semibold text-gray-500 mb-2 uppercase tracking-wide">Core Capability</h4>
        <p className="text-gray-700 leading-relaxed">{capability.coreCapability}</p>
      </div>

      {/* Evidence Metrics */}
      <div className="mb-6">
        <h4 className="text-sm font-semibold text-gray-500 mb-3 uppercase tracking-wide">Evidence (Core)</h4>
      ...
      {/* Target Audience */}
      <div className="mb-6">
        <h4 className="text-sm font-semibold text-gray-500 mb-2 uppercase tracking-wide">Target Audience</h4>
        <p className="text-gray-600 text-sm leading-relaxed">{capability.targetAudience}</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {capability.endpoint && (
          <button 
            className={`flex items-center gap-2 px-4 py-2 ${theme.bg} ${theme.text} rounded-lg text-sm font-medium hover:${theme.hover} transition-colors duration-300`}
        ...
        <div className={`flex items-center gap-2 px-4 py-2 ${theme.bg} ${theme.text} rounded-lg text-sm font-medium`}>
      ...
      {capability.quickTest && (
        <div className="mt-6 p-4 bg-gray-50 rounded-lg">
          <h5 className="text-sm font-semibold text-gray-700 mb-2">Quick Test</h5>
