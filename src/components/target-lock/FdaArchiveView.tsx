    ...
    <div className="flex flex-col gap-8">
      ...
      <div className={`border rounded-sm overflow-hidden shadow-2xl ${isDarkMode ? 'bg-zinc-950/40 border-zinc-900' : 'bg-white border-slate-200'}`}>
        ...
        {TWO_LAYER_MATRIX.map((row, i) => (
          <div key={i} className={`flex items-center px-8 py-5 border-b transition-colors ${
            row.isCritical
            ...
            <span className={`text-[14px] font-black w-24 ${row.l1 === 'HIGH' ? 'text-emerald-500' : 'text-rose-500'}`}>{row.l1}</span>
            <span className={`text-[14px] font-black w-24 ${row.l2 === 'HIGH' ? 'text-emerald-500' : 'text-rose-500'}`}>{row.l2}</span>
            <span className={`text-[13px] font-bold flex-1 ${isDarkMode ? 'text-zinc-300' : 'text-slate-700'}`}>{row.prediction}</span>
            {row.cost && <span className="text-[14px] font-black text-rose-500">{row.cost}</span>}
          </div>
        ))}
      </div>
      ...
      <div className={`border rounded-sm overflow-hidden shadow-2xl ${isDarkMode ? 'bg-zinc-950/40 border-zinc-900' : 'bg-white border-slate-200'}`}>
        <div className={`flex items-center justify-between px-8 py-6 border-b ${isDarkMode ? 'border-zinc-900' : 'border-slate-100'}`}>
          <div className="flex items-center gap-3">
            ...
            <span className={`text-[12px] font-black uppercase tracking-[0.4em] ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>LATIFY Chain of Custody</span>
          </div>
        ...
        <div className="grid grid-cols-3 gap-8 p-8">
          <div className={`p-6 border rounded transition-colors ${isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-slate-200 shadow-sm'}`}>
          ...
          <div className={`p-6 border rounded text-center transition-colors ${isDarkMode ? 'bg-rose-500/5 border-rose-500/20' : 'bg-rose-50 border-rose-200'}`}>
            ...
            <span className={`text-[11px] font-mono block mt-3 ${isDarkMode ? 'text-zinc-500' : 'text-slate-700'}`}>Production cosine_sim()</span>
          </div>
        </div>

        {/* Citation — from receipt */}
        <div className={`px-8 pb-6 border-t ${isDarkMode ? 'border-zinc-900' : 'border-slate-100'}`}>
          <p className={`text-[11px] font-mono leading-relaxed pt-4 ${isDarkMode ? 'text-zinc-500' : 'text-slate-500'}`}>
      ...
      <div className="grid grid-cols-2 gap-8">
        ...
        <div className={`border rounded-sm overflow-hidden shadow-2xl ${isDarkMode ? 'bg-zinc-950/40 border-zinc-900' : 'bg-white border-slate-200'}`}>
          <div className={`flex items-center justify-between px-8 py-6 border-b ${isDarkMode ? 'border-zinc-900' : 'border-slate-100'}`}>
            ...
            <span className="text-lg font-black text-emerald-500">{FDA_STATS.retroConcordance}</span>
          </div>
          {FDA_RETROACTIVE.map((entry, i) => (
            <motion.div key={entry.gene} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
              className={`flex items-center justify-between px-8 py-3 border-b border-l-2 border-l-emerald-500/40 ${isDarkMode ? 'border-zinc-900 hover:bg-white/[0.02]' : 'border-slate-50 hover:bg-slate-50'}`}
            >
              <div className="flex items-center gap-4">
                <span className={`text-[12px] font-black uppercase w-16 ${isDarkMode ? 'text-cyan-400' : 'text-indigo-600'}`}>{entry.gene}</span>
                <span className={`text-[12px] ${isDarkMode ? 'text-zinc-300' : 'text-slate-700'}`}>{entry.drug}</span>
              </div>
              <div className="flex items-center gap-3">
        ...
        <div className={`border rounded-sm overflow-hidden shadow-2xl ${isDarkMode ? 'bg-zinc-950/40 border-zinc-900' : 'bg-white border-slate-200'}`}>
          <div className={`flex items-center justify-between px-8 py-6 border-b ${isDarkMode ? 'border-zinc-900' : 'border-slate-100'}`}>
            <div className="flex items-center gap-3">
              <ArrowRight className={`w-5 h-5 ${isDarkMode ? 'text-cyan-500' : 'text-indigo-600'}`} />
            ...
            <span className={`text-[11px] font-bold uppercase tracking-widest ${isDarkMode ? 'text-zinc-400' : 'text-slate-400'}`}>Immutable</span>
          </div>
          {FDA_PROSPECTIVE.map((entry, i) => (
            ...
            >
              <div className="flex items-center gap-4">
                <span className={`text-[12px] font-black uppercase w-20 ${entry.highlight ? 'text-rose-500' : (isDarkMode ? 'text-cyan-400' : 'text-indigo-600')}`}>{entry.gene}</span>
                <span className={`text-[12px] ${isDarkMode ? 'text-zinc-300' : 'text-slate-700'}`}>{entry.drug}</span>
              </div>
      ...
      <div className={`p-8 border rounded-sm text-center transition-colors ${isDarkMode ? 'bg-rose-500/5 border-rose-500/20' : 'bg-rose-50 border-rose-200'}`}>
      ...
      <div className={`border rounded-sm overflow-hidden shadow-2xl ${isDarkMode ? 'bg-zinc-950/40 border-zinc-900' : 'bg-white border-slate-200'}`}>
        ...
        {DRUG_CORRECTIONS.map((c) => (
          <div key={c.gene} className={`flex items-center px-8 py-4 border-b border-l-2 border-l-amber-500/40 ${isDarkMode ? 'border-zinc-900' : 'border-slate-50'}`}>
            <span className={`text-[12px] font-black uppercase w-24 shrink-0 ${isDarkMode ? 'text-amber-400' : 'text-amber-600'}`}>{c.gene}</span>
            <span className={`text-[12px] line-through flex-1 ${isDarkMode ? 'text-rose-400/60' : 'text-rose-400'}`}>{c.error}</span>
            <span className={`text-[12px] font-bold flex-1 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>{c.correction}</span>
            <span className={`text-[11px] font-bold w-32 text-right shrink-0 ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>{c.source}</span>
          </div>
        ))}
      </div>
    </div>
