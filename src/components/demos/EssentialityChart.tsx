    ...
    <div className="space-y-2">
      {series.map((p, i) => (
        <div key={i} className="flex items-center justify-between gap-4 text-sm">
          <div className="min-w-[140px] text-slate-200">{p.context.cellLine}{p.context.mutations && p.context.mutations.length ? ` (${p.context.mutations.join(',')})` : ''}</div>
          ...
          <div className="font-mono w-12 text-right text-slate-100">{p.score.toFixed(2)}</div>
        </div>
      ))}
    </div>
  </div>
