...
const OracleExplainTrack: React.FC<Props> = ({ sequence, variant, saeFeatures, deltaLLSeries, className }) => {
  ...
  return (
    <div className={`w-full rounded-xl border border-slate-700 p-4 bg-slate-800 ${className || ''}`}>
      <div className="mb-2 text-sm text-slate-300 flex items-center justify-between">
        <span>Oracle Explain</span>
        <span className="font-mono text-xs text-slate-400">variant: {`${variant.ref}${variant.pos}>${variant.alt}`}</span>
      </div>
      <div className="h-24 relative overflow-hidden bg-slate-900 rounded">
        {bars.map(b => (
          <div key={b.x} className="absolute bottom-0" style={{ left: b.x * barWidth, width: barWidth, height: 2, background: '#64748b' }} />
        ))}
        {saeFeatures.map((f, i) => (
      ...
      <div className="mt-3 h-24 relative overflow-hidden bg-slate-900 rounded">
      ...
      <div className="mt-2 text-xs text-slate-400">Top ribbon = learned features (exon/intron/TF). Bottom chart = Δ likelihood around the variant.</div>
    </div>
  );
};
