...
export const ArtifactRow: React.FC<ArtifactRowProps> = ({ doc, path, slug, type, status, isDarkMode }) => {
  ...
  return (
    <Tag
      {...linkProps}
      className={`group flex flex-col gap-2 sm:grid sm:grid-cols-12 sm:gap-4 py-4 border-b transition-all px-2 sm:px-3 min-w-0 ${
        slug ? 'cursor-pointer' : ''
      } ${isDarkMode ? 'border-zinc-900/50 hover:bg-cyan-500/5' : 'border-slate-100 hover:bg-indigo-50/50'}`}
    >
      <div className="sm:col-span-5 flex items-center gap-3 sm:gap-4 min-w-0">
        ...
        <span className={`text-[11px] sm:text-[12px] font-bold transition-colors min-w-0 ${isDarkMode ? 'text-zinc-300 group-hover:text-white' : 'text-slate-700 group-hover:text-indigo-900'}`}>{doc}</span>
      </div>
      <div className="sm:col-span-3 flex items-center font-mono min-w-0 pl-11 sm:pl-0">
        <span className={`text-[9px] sm:text-[10px] break-all sm:truncate sm:max-w-[180px] ${isDarkMode ? 'text-zinc-300' : 'text-slate-600'}`}>{path}</span>
      </div>
      <div className="sm:col-span-4 flex items-center justify-start sm:justify-end gap-2 sm:gap-3 flex-wrap pl-11 sm:pl-0">
        {type && tc && (
...
export const ArtifactList: React.FC<{ artifacts: ArtifactEntry[]; isDarkMode: boolean }> = ({ artifacts, isDarkMode }) => (
  <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide min-h-0">
    {artifacts.map((art, i) => (
      <ArtifactRow key={i} doc={art.doc} path={art.path} slug={art.slug} type={art.type} status={art.status} isDarkMode={isDarkMode} />
    ))}
  </div>
);
