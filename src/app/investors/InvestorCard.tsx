...
const InvestorCard: React.FC<InvestorCardProps> = ({ title, description, href, iconName, status }) => {
  ...
  const cardContent = (
    <div
      className={`
        bg-slate-800/50 border border-slate-700 rounded-xl p-6 flex flex-col h-full
    ...
    >
      <div className="flex-grow">
        <div className="mb-4 flex items-center gap-4">
          <div className="bg-slate-700 p-3 rounded-lg">
            <Icon className={`w-6 h-6 ${isLocked ? 'text-slate-500' : 'text-blue-400'}`} />
          </div>
      ...
      <div className="mt-6 flex items-center justify-between">
        {isLocked ? (
          ...
          <div className="flex items-center gap-2 text-sm text-blue-400 font-semibold group">
