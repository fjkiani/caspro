...
export const metadata: Metadata = {
  ...
  description: 'Long-form scientific manuscripts and evidence PDFs from CrisPRO.',
};

export default async function ManuscriptsListPage() {
  ...
  return (
    <div className="min-h-screen font-mono bg-gradient-to-br from-white via-slate-50 to-white dark:bg-[#020408] dark:bg-none dark:text-zinc-100 relative">
      ...
      <header className="bg-white/95 dark:bg-zinc-950/80 border-b border-slate-200 dark:border-zinc-800 sticky top-14 z-40 shadow-sm backdrop-blur-md relative">
        <div className="flex items-center justify-between p-4 max-w-5xl mx-auto">
          ...
          <span className="text-sm text-slate-500 dark:text-zinc-500">Manuscripts</span>
        </div>
      </header>

      <main className="max-w-4xl mx-auto px-4 py-12 relative">
        <h1 className="text-3xl font-black tracking-tight text-slate-900 dark:text-zinc-100 mb-2">Manuscripts</h1>
        {/* <p className="text-slate-600 dark:text-zinc-400 mb-10">
          Long-form PDFs from Hygraph. Entries use <span className="font-medium text-slate-700 dark:text-zinc-300">MediaItem.type = PDF</span>; blog decks use{' '}
          <span className="font-medium text-slate-700 dark:text-zinc-300">DECK</span> and are not listed here.
        </p> */}
        ...
        {withSlug.length === 0 ? (
          <div className="rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950/60 p-8 text-center text-slate-600 dark:text-zinc-400">
            ...
            <p className="mt-1 text-sm">
              In Hygraph, set each manuscript entry&apos;s <code className="text-xs">type</code> to{' '}
              <code className="text-xs">PDF</code>, attach a PDF asset, publish, and ensure the API can read{' '}
              <code className="text-xs">mediaItems</code>.
            </p>
          </div>
        ) : (
